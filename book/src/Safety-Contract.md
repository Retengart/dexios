...
| ARCH-001 | Archive boundaries | Pack streams ZIP bytes directly into V1 encryption, unpack temporary ZIP artifacts remain plaintext exposure, archive structure is bounded by `ArchiveLimits`, and pack/unpack must not be described as secure erase. | current | Current source, real filesystem tests, source gates, and editable docs | `dexios-domain/src/archive.rs`; `dexios-domain/src/pack.rs`; `dexios-domain/src/unpack.rs`; `dexios-core/src/stream.rs`; `dexios-domain/tests/pack_paths.rs`; `dexios-domain/tests/unpack.rs`; `dexios-core/tests/stream_v1.rs`; `dexios/tests/pack_cli_regressions.rs`; `dexios/tests/unpack_cli_regressions.rs`; `book/src/technical-details/Directory-Packing.md`; `book/src/technical-details/Secure-Erase.md` | SAFE-01, ARCH-04, ARCH-05, VERI-03, PERF-03 | Phase 6 / Phase 12 |
...
| pack/unpack archive boundaries | Current source/tests plus editable docs are the authority for checked unpack construction, ZIP boundary behavior, plaintext temp exposure, Zstd-by-default offline archival framing, minimal metadata preservation, and structural archive limits. | `dexios-domain/src/archive.rs`; `dexios-domain/src/pack.rs`; `dexios-domain/src/unpack.rs`; `dexios-core/src/stream.rs`; `dexios/src/subcommands/unpack.rs`; `dexios-domain/tests/pack_paths.rs`; `dexios-domain/tests/unpack.rs`; `dexios-domain/tests/archive_public_api.rs`; `dexios-core/tests/stream_v1.rs`; `dexios/tests/pack_cli_regressions.rs`; `dexios/tests/unpack_cli_regressions.rs`; `book/src/technical-details/Directory-Packing.md`; `book/src/technical-details/Secure-Erase.md`; `book/src/` editable documentation source. | `spec/specification-v1.pdf` contains older erase wording; generated `docs/` is generated output. | Treat `UnpackIntent` as the public checked unpack boundary. Pack-side plaintext temporary ZIP exposure is reduced by streaming ZIP bytes into V1 encryption. Unpack-side plaintext temporary ZIP exposure remains ordinary temp-file cleanup, not secure erase. Treat `ArchiveLimits` as the current structural policy: 100000 entries, 4096 normalized path bytes, and 64 normalized path components. |
