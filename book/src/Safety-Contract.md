...
| STOR-003 | Filesystem transactions | Staged writes use same-directory temporary files and staged flush/sync/persist before final placement: `NamedStagedOutput::prepare_for_persist` requires a write, calls `flush`, calls `File::sync_all`, and only then uses `tempfile::NamedTempFile::persist` or `persist_noclobber` through `persist_prepared`. Linked commits prepare every staged artifact before any persist call. Partial commit evidence reports already committed artifacts and the failed artifact; committed outputs are not rolled back. Dexios does not claim portable parent-directory durability across every filesystem or platform. | current | Current source, source gates, and real filesystem tests | `dexios-domain/src/storage/temp.rs`; `dexios-domain/src/storage/transaction.rs`; `dexios-domain/tests/transactions.rs`; `dexios/tests/verification_gate_docs.rs`; `book/src/technical-details/Directory-Packing.md`; `book/src/technical-details/Secure-Erase.md` | FS-01, FS-03, FS-04, FS-05 | Phase 11 |
| STOR-004 | Cleanup identity and deletion | Cleanup is ordinary delete-after-success cleanup after complete commit and requested hash success. Cleanup receipts capture a target identity snapshot, changed cleanup identity fails closed, and cleanup failures are reported after commit without reverting outputs. `remove_file` does not guarantee immediate physical deletion, and Dexios does not claim physical sanitization. | current | Current source, source gates, and real filesystem tests | `dexios-domain/src/storage/cleanup.rs`; `dexios-domain/tests/cleanup_receipts.rs`; `dexios/tests/delete_source_cli.rs`; `dexios/tests/storage_transactions_cli.rs`; `dexios/tests/verification_gate_docs.rs`; `book/src/technical-details/Secure-Erase.md` | FS-02, FS-04, FS-05 | Phase 11 |
