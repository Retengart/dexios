...
| ARCH-001 | Archive boundaries | Current pack streams archive bytes directly into V1 encryption, current unpack temporary ZIP artifacts remain plaintext exposure, and canonical V1 reserves a Dexios-owned manifest-first payload framing contract with structural limits and ordered body frames rather than ZIP-as-format surface. | current | Current source, real filesystem tests, source gates, and editable docs | `dexios-domain/src/archive.rs`; `dexios-domain/src/pack.rs`; `dexios-domain/src/unpack.rs`; `dexios-core/src/payload.rs`; `dexios-core/src/stream.rs`; `dexios-domain/tests/pack_paths.rs`; `dexios-domain/tests/unpack.rs`; `dexios-core/tests/stream_v1.rs`; `dexios/tests/pack_cli_regressions.rs`; `dexios/tests/unpack_cli_regressions.rs`; `book/src/technical-details/Directory-Packing.md`; `book/src/technical-details/Secure-Erase.md` | FMT-01, STRM-04, ARCH-04, ARCH-05, PERF-03 | Phase 3 / Phase 12 |
...
| pack/unpack archive boundaries | Current source/tests plus editable docs are the authority for checked unpack construction, current ZIP boundary behavior, plaintext temp exposure, canonical manifest-first payload framing, minimal metadata preservation, and structural archive limits. | `dexios-domain/src/archive.rs`; `dexios-domain/src/pack.rs`; `dexios-domain/src/unpack.rs`; `dexios-core/src/payload.rs`; `dexios-core/src/stream.rs`; `dexios/src/subcommands/unpack.rs`; `dexios-domain/tests/pack_paths.rs`; `dexios-domain/tests/unpack.rs`; `dexios-domain/tests/archive_public_api.rs`; `dexios-core/tests/stream_v1.rs`; `dexios/tests/pack_cli_regressions.rs`; `dexios/tests/unpack_cli_regressions.rs`; `book/src/technical-details/Directory-Packing.md`; `book/src/technical-details/Secure-Erase.md`; `book/src/` editable documentation source. | `spec/specification-v1.pdf` contains older erase wording; generated `docs/` is generated output. | Treat `UnpackIntent` as the public checked unpack boundary. Current pack-side plaintext temporary ZIP exposure is reduced by streaming archive bytes into V1 encryption. Current unpack-side plaintext temporary ZIP exposure remains ordinary temp-file cleanup, not secure erase. Canonical V1 archive framing is Dexios-owned manifest-first `DXAR` plus ordered body frames; ZIP crate bytes and types are not canonical format surface. |
