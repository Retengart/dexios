...
| STOR-003 | Filesystem transactions | Staged writes use same-directory temporary files and staged flush/sync/persist before final placement: `NamedStagedOutput::prepare_for_persist` requires a write, calls `flush`, calls `File::sync_all`, and only then uses `tempfile::NamedTempFile::persist` or `persist_noclobber` through `persist_prepared`. Linked commits prepare every staged artifact before any persist call. Partial commit evidence reports already committed artifacts and the failed artifact; committed outputs are not rolled back. Dexios does not claim portable parent-directory durability across every filesystem or platform, and this is no full power-failure proof. | current | Current source, source gates, and real filesystem tests | `dexios-domain/src/storage/temp.rs`; `dexios-domain/src/storage/transaction.rs`; `dexios-domain/tests/transactions.rs`; `dexios/tests/verification_gate_docs.rs`; `book/src/technical-details/Directory-Packing.md`; `book/src/technical-details/Secure-Erase.md` | FS-01, FS-03, FS-04, FS-05 | Phase 11 |
