...
| KEY-003 | Key material | canonical V1 key add, change, delete, and verify are proof-gated fixed-physical-slot operations; add/change persist fresh keyslot wrapping nonces and preserve payload bytes. | current | Domain generated fixture tests | `dexios-domain/tests/keyslots_v1.rs`; `dexios/tests/key_cli_regressions.rs`; `dexios-domain/src/key/add.rs`; `dexios-domain/src/key/change.rs`; `dexios-domain/src/key/delete.rs`; `dexios-domain/src/key/verify.rs` | FMT-04, FMT-05, VERI-02 | Phase 3 |
...
| STOR-003 | Filesystem transactions | Staged writes use same-directory temporary files and staged flush/sync/persist before final placement: `NamedStagedOutput::prepare_for_persist` requires a write, calls `flush`, calls `File::sync_all`, and only then uses `tempfile::NamedTempFile::persist` or `persist_noclobber` through `persist_prepared`. Linked commits prepare every staged artifact before any persist call. During manifest unpack, if the first selected file commit fails after selected directories were created, Dexios best-effort removes directories created by the current post-final-auth directory pass before first selected file commit; pre-existing directories are preserved. Partial commit evidence reports already committed artifacts and the failed artifact; committed outputs are not rolled back. Dexios does not claim portable parent-directory durability across every filesystem or platform, and this is no full power-failure proof. | current | Current source, source gates, and real filesystem tests | `dexios-domain/src/storage/temp.rs`; `dexios-domain/src/storage/transaction.rs`; `dexios-domain/src/storage/cleanup.rs`; `dexios-domain/tests/transactions.rs`; `dexios-domain/tests/unpack.rs`; `dexios/tests/verification_gate_docs.rs`; `book/src/technical-details/Directory-Packing.md`; `book/src/technical-details/Secure-Erase.md` | FS-01, FS-03, FS-04, FS-05, TXN-01, TXN-02, TXN-03 | Phase 11 / Phase 12 |
| STOR-004 | Cleanup identity and deletion | Cleanup is ordinary delete-after-success cleanup after complete commit and requested hash success. Cleanup receipts capture a target identity snapshot, changed cleanup identity fails closed, and cleanup failures are reported after commit without reverting outputs. `remove_file` does not guarantee immediate physical deletion, and Dexios claims no secure erase and no physical sanitization. | current | Current source, source gates, and real filesystem tests | `dexios-domain/src/storage/cleanup.rs`; `dexios-domain/tests/cleanup_receipts.rs`; `dexios/tests/delete_source_cli.rs`; `dexios/tests/storage_transactions_cli.rs`; `dexios/tests/verification_gate_docs.rs`; `book/src/technical-details/Secure-Erase.md` | FS-02, FS-04, FS-05 | Phase 11 |
| ARCH-001 | Archive boundaries | Current pack and unpack use Dexios-owned manifest-first archive payloads with `DXAR` manifests and ordered `DXBF` body frames. ZIP bytes and ZIP crate types are not canonical V1 archive surface. Normal operation no longer creates a full plaintext archive temporary file; the current archive workflow has no full plaintext archive temporary file. Unpack-side plaintext exposure is scoped to selected staged file bodies and ordinary filesystem temporary/staged files, without secure erase or capacity-preflight claims. | current | Current source, real filesystem tests, source gates, and editable docs | `dexios-domain/src/archive.rs`; `dexios-domain/src/pack.rs`; `dexios-domain/src/unpack.rs`; `dexios-core/src/payload.rs`; `dexios-core/src/stream.rs`; `dexios-domain/tests/pack_paths.rs`; `dexios-domain/tests/unpack.rs`; `dexios-core/tests/stream_v1.rs`; `dexios/tests/pack_cli_regressions.rs`; `dexios/tests/unpack_cli_regressions.rs`; `book/src/technical-details/Directory-Packing.md`; `book/src/technical-details/Secure-Erase.md` | FMT-01, STRM-04, ARCH-04, ARCH-05, PERF-03 | Phase 5 / Phase 6 |
...
| `canonical-v1-wrong-key` | `wrong-key` | `dexios-domain/tests/keyslots_v1.rs` | VERI-02 | KEY-002 |
...
| `keyslot-mutation-two-keyslots` | `keyslot-mutation` | `dexios-domain/tests/keyslots_v1.rs`; `dexios-core/tests/testdata/fixture_manifest.toml` | FMT-04 | KEY-003 |
...
identity, tracked dirty state, `Cargo.lock` SHA256, tool versions, Cargo
metadata, the verification command contract, and optional asset SHA256 hashes.
...
property not covered by separately completed gate results.
