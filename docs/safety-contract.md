...
| FMT-003 | Headers | Header restore rejects short targets before writing or extending them. | broken | Current source analysis plus ignored regression | `dexios-domain/src/header/restore.rs` uses a partial read before writing; `dexios-domain/tests/header_restore.rs::quarantined_known_bug_header_restore_rejects_short_target` | FORM-05, VERI-01 | Phase 5 |
...
| KEY-001 | Key material | Committed encrypted artifacts should not end with zero usable keyslots. | broken | Current source analysis plus ignored regression | `dexios-domain/src/key/delete.rs` removes the matching keyslot before writing the new header; `dexios-domain/tests/keyslots_v1.rs::quarantined_known_bug_key_del_rejects_final_keyslot` | SAFE-01, KEY-01, KEY-02, VERI-01 | Phase 5 |
...
| KEY-003 | Key material | V1 keyslot mutation workflows preserve a remaining decrypting key when another usable keyslot exists. | current | Domain generated fixture test | `dexios-domain/tests/keyslots_v1.rs::can_add_verify_change_and_delete_v1_keyslots` | SAFE-01, VERI-02 | Phase 5 |
| STOR-001 | Storage writes | Existing final outputs should remain unchanged when an operation fails. | broken | Current source analysis plus ignored regression | `dexios-domain/src/storage.rs`; `dexios/src/subcommands/encrypt.rs`; `dexios/src/subcommands/decrypt.rs`; `dexios/tests/decrypt_cli_regressions.rs::quarantined_known_bug_decrypt_wrong_key_preserves_existing_output` | SAFE-01, STOR-01, STOR-02, VERI-01 | Phase 4 |
| STOR-002 | Path identity | Input/output path aliases should be rejected before final output handles open. | broken | Current source analysis plus ignored regression | `dexios/src/subcommands/encrypt.rs` and `dexios/src/subcommands/decrypt.rs` reject equal strings but do not prove resolved path identity before opening outputs; `dexios/tests/encrypt_cli_regressions.rs::quarantined_known_bug_encrypt_rejects_same_file_alias_before_opening_output` | SAFE-01, STOR-03, VERI-01 | Phase 4 |
...
| header dump/strip/restore | Current source and owning tests are the authority. | `dexios/src/subcommands/header.rs`; `dexios-domain/src/header/`; `dexios-domain/tests/header_restore.rs::quarantined_known_bug_header_restore_rejects_short_target`; `book/src/dexios-core/Headers.md`; `book/src/` editable documentation source. | `spec/specification-v1.pdf` is historical input; generated `docs/` is generated output. | Treat restore exactness and partial-read behavior as a broken Phase 5 workflow invariant until the quarantined regression is unignored or replaced. |
...
|-----------|-----------------|----------|-----------|
| FMT-003 | header restore partial reads: Restoring a dumped header into a short zero-filled target can extend the target instead of rejecting the partial read. | Current source analysis in `dexios-domain/src/header/restore.rs`; `dexios-domain/tests/header_restore.rs::quarantined_known_bug_header_restore_rejects_short_target`. | Phase 5 |
| KEY-001 | final keyslot deletion: Deleting the final usable keyslot can create an encrypted artifact with no usable keyslots. | Current source analysis in `dexios-domain/src/key/delete.rs`; `dexios-domain/tests/keyslots_v1.rs::quarantined_known_bug_key_del_rejects_final_keyslot`. | Phase 5 |
| STOR-001 | output truncation on failure: Workflows can open and truncate existing final outputs before the operation has fully succeeded. | Current source analysis in CLI/domain storage output-open paths; `dexios/tests/decrypt_cli_regressions.rs::quarantined_known_bug_decrypt_wrong_key_preserves_existing_output`. | Phase 4 |
| STOR-002 | same-file alias truncation: Same-file checks are string-level in current CLI encrypt/decrypt paths and do not prove canonical, symlink, or hardlink identity before output handles open. | Current source analysis in `dexios/src/subcommands/encrypt.rs` and `dexios/src/subcommands/decrypt.rs`; `dexios/tests/encrypt_cli_regressions.rs::quarantined_known_bug_encrypt_rejects_same_file_alias_before_opening_output`. | Phase 4 |
...
| `keyslot-mutation-two-keyslots` | `keyslot-mutation` | `dexios-domain/tests/keyslots_v1.rs::can_add_verify_change_and_delete_v1_keyslots` | VERI-02 | KEY-003 |
