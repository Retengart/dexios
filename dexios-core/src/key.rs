...
pub fn balloon_hash(
    ...
    let params = match version {
        HeaderVersion::V1 | HeaderVersion::V2 | HeaderVersion::V3 | HeaderVersion::V4 => {
            balloon_hash::Params::new(262_144, 1, 1)
...
pub fn decrypt_master_key(
    ...
    match header.header_type.version {
        HeaderVersion::V1 | HeaderVersion::V2 | HeaderVersion::V3 => {
            // This case is no longer possible as argon2id_hash is removed
            // Keeping it for now to avoid breaking existing code, but it will always return an error
            Err(anyhow::anyhow!("argon2id_hash is no longer supported."))
        }
        HeaderVersion::V4 => {
